<template>
  <q-list separator bordered class="rounded-borders"
    ...
    <q-item v-if="this.typeRecap == 'check'">
      ...
      <q-item-section v-if="element.state == '2'">
    ...
    <q-item v-if="this.typeRecap == 'active'">
      ...
      <q-item-section v-if="element.state == '3' || element.state == '4'">
        <div class="q-mt-md text-right">
          <q-btn label="Delivery" color="primary" @Click="finishCommandes(element.id)"/>
        </div>
      </q-item-section>
...
export default defineComponent ({
  ...
  methods:{
    ...
    {
      api.patch("commandes/"+idCmd, { state : 3, idLivreur: Cookies.get('current_id') })
        .then(() => {
    ...
    {
      api.patch("commandes/"+idCmd, { state : 99 })
        .then(() => {
          console.log("Perfect Update!");
          location.reload();
        })
